...
# a recent laptop is a few minutes with the defaults; pass `--fast`
# to skip the heavier fixtures.
#
# Usage:
#   julia --project=benchmark benchmark/simple/runbench.jl
#   julia --project=benchmark benchmark/simple/runbench.jl --fast

import Pkg
Pkg.activate(joinpath(@__DIR__, ".."))

const JULIA_BIN     = first(Base.julia_cmd().exec)
const BENCH_PROJECT = joinpath(@__DIR__, "..")

fast_mode = "--fast" in ARGS

# Per-bench argument lists. `--fast` trims ferh and shrinks the per-bench
# wall-clock budget so the runner finishes in well under a minute for
# quick smoke checks.
benches = if fast_mode
    [
        ("bench_construction.jl", ["--fixtures=bcc,fege", "--seconds=0.3"]),
        ("bench_energy.jl",       ["--fixtures=bcc,fege", "--seconds=0.3"]),
        ("bench_sweep.jl",        ["--fixtures=bcc",      "--seconds=0.5"]),
        ("bench_compare.jl",      ["--fixtures=bcc",      "--seconds=0.3"]),
    ]
else
    [
...
for (script, extra) in benches
    ...
    cmd = `$JULIA_BIN --project=$BENCH_PROJECT $path $extra`
    println()
    println("-" ^ 78)
    println("\$ ", cmd)
    println("-" ^ 78)
