...
2. 同フォルダに以下 3 ファイルを置き、初稿を書く:
   - `requirements.md` — 目的・不変条件・スコープ・完了基準
   - `design.md` — モジュール構成・API・型・規約
   - `tasklist.md` — マイルストーン (粗い粒度。日々の細かい作業は TaskCreate)
3. **3 ファイルが揃った段階で `spec-reviewer` サブエージェントを起動し、対象フォルダの
   パスを渡してレビューさせる**。レビュー観点は個別品質・3 ファイル間の整合性・
   CLAUDE.md 規約 (物理規約/言語/連動箇所)・既存コードとの整合性。
4. spec-reviewer のレポートを user に提示し、指摘を踏まえて user と相談しながら 3 ファイル
   を修正する (修正は spec-reviewer ではなくメインエージェントが行う)。
5. spec の合意ができてから実装に着手。
6. 完了後もフォルダは残す (削除しない、履歴として参照)。
