...
# Phase-2 un-fold path types (general supercell matrix M). Defined before
# LocalEnergyTemplate, which embeds them. See the construction block lower in
# this file for the geometry (placement onto M via SupercellCommon).
...
struct LocalEnergyTemplate
    ...
    # Phase-2 un-fold path (general supercell matrix). `unfold === nothing` on the
    # legacy folded `repeat` path (the folded fields above are used instead);
    # otherwise `prim_templates` + `unfold` drive `_template_local_energy!` and the
    # folded fields are empty.
    prim_templates::Vector{PrimClusterTemplate}
    unfold::Union{Nothing, UnfoldSAITable}
end
...
On the legacy folded `repeat` path (`h.supercell_matrix === nothing`) this holds
one `BaseClusterInstance` per unique base-cell cluster with per-atom lookup
tables for on-the-fly tile-major supercell index reconstruction during `sweep!`.

On the general supercell-matrix (un-fold) path it holds primitive-cell
`PrimClusterTemplate`s and a per-atom cell-major `UnfoldSAITable`; the folded
fields are empty and `_template_local_energy!` takes the un-fold branch.
"""
function build_local_energy_template(h::SCEHamiltonian)::LocalEnergyTemplate
    if h.supercell_matrix !== nothing
        return _build_local_energy_template_matrix(h)
    end
    ...
    return LocalEnergyTemplate(
        ...
        PrimClusterTemplate[], nothing,
    )
end

"""
Build a `LocalEnergyTemplate` for the general supercell-matrix (un-fold) path:
primitive-cell templates (`_build_prim_cluster_templates`) plus a per-atom
cell-major de-duplicated instance table (`_build_sai_table_cellmajor`). The
folded fields are left empty; `_template_local_energy!` dispatches on
`unfold !== nothing`.
"""
function _build_local_energy_template_matrix(h::SCEHamiltonian)::LocalEnergyTemplate
    templates, related_by_subl = _build_prim_cluster_templates(h)
    unfold = _build_sai_table_cellmajor(templates, related_by_subl, h)
    empty_related = Vector{RelatedBaseCluster}[]
    return LocalEnergyTemplate(
        BaseClusterInstance[], BaseClusterInstance2[], BaseClusterInstance3[],
        empty_related, empty_related, empty_related,
        Int[], Int[], Int[], Int[],
        templates, unfold,
    )
end
...
the deduped/accumulated instance list of `_build_cluster_instances_matrix`, so
