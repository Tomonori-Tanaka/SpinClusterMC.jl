...
The Simple Hamiltonian tiles the cluster interactions read from `jphi.xml` onto
a supercell. Historically this was restricted to integer *diagonal* multiples of
the XML "base cell" (`repeat = (n1, n2, n3)`). This file generalizes the tiling
to an arbitrary integer supercell matrix `M` (3×3, `det(M) != 0`) expressed in
units of the *primitive* cell, mirroring how Sunny.jl reshapes a primitive cell
onto a general supercell.

This part of the file (M1) provides the integer-linear-algebra primitives used
by the later stages (primitive extraction, cluster templating, tiling):

  - `_int_det3`                    — exact 3×3 integer determinant.
  - `_adjugate3`                   — 3×3 adjugate (`M * adj(M) = det(M) I`).
  - `_col_hermite`                 — column Hermite normal form `M = H * U`.
  - `_wrap_offset_into_supercell`  — fold an integer cell offset into the
                                     fundamental domain of `M`'s column lattice.

All operations are pure integer arithmetic (no floating point), so they are
exact for any sign of `det(M)`.
"""
...
cell's discrete translation table (`SystemData.map_sym` / `n_trans`).
...
- `prim_to_base`: inverse map `(s, Δ) => a` for the base atoms (the `n_trans`
  cells that fill the base cell). Used by the diagonal backward-compatibility
  layer.
...
Recover the primitive cell from a base-cell `SystemData`, following the same
construction as Magesty's `SunnyExport._sunny_primitive`.
...
"""
function extract_primitive(sys::SystemData)::PrimitiveCell
    lattice = sys.lattice
    pos_frac = sys.pos_frac
    base_n = sys.n_atoms
    n_trans = sys.n_trans
    map_sym = sys.map_sym
...
- `ls`, `Lf`, `Lseq`, `weights`, `J`, `multiplicity`: identical meaning to the
  corresponding `ClusterInstance` / `SALCBasisData` fields.
"""
...
# for general supercell tiling (see `build_templates`).
...
Each site `k` of a basis is re-expressed relative to the pivot (site 1) as a
`(sublattice, primitive-cell offset)` pair, with the offset being the actual
listed displacement in primitive-cell units,

    Δ_k = base_to_prim[atom_k].offset - base_to_prim[atom_1].offset,

and `Δ_1 == (0, 0, 0)`. No base-cell translation loop is needed: the supercell
tiling (`_generate_instances_matrix`) regenerates the copies that the diagonal
`_generate_instances` produces by enumerating translations.
...
where `s_base = _cluster_base_stabilizer(...)`. This keeps the energy density
identical to the base-cell model for an arbitrary supercell matrix `M`
(base-multiple, non-multiple, non-diagonal, or even a single primitive cell),
matching the Magesty convention `E = Σ contract · multiplicity` exactly.
...
# the HNF diagonal box. Returns (cell_index, cells_by_id) where
# cell_index[wrapped_offset] = cell_id and cells_by_id[cell_id] = wrapped_offset.
...
(`_wrap_offset_into_supercell`), and the partner atom index is looked up. A
per-template `seen` set deduplicates by sorted atom indices, mirroring the
per-basis deduplication of the diagonal-only `_generate_instances` (so a cluster
that wraps onto itself in a small supercell is still counted once).

Atoms use a primitive cell-major numbering
`super_index(cell_id, subl) = subl + n_prim * (cell_id - 1)`, with
`n_atoms = n_prim * |det(M)|`. This is independent of the legacy base-cell
numbering; the diagonal `repeat` API keeps using the unchanged
`_generate_instances` for exact backward compatibility, while this path serves
the general `supercell_matrix` API.
...
reproduces the base-cell energy density on any `M`.
"""
